...
function! GitDiff(args)
    let git_output = s:SystemGit('diff ' . a:args . ' -- ' . shellescape(s:Expand('%')))
...
function! GitLog(args)
    let git_output = s:SystemGit('log ' . a:args . ' -- ' . shellescape(s:Expand('%')))
    call <SID>OpenGitBuffer(git_output)
...
function! GitCommit(args)
    ...
    augroup GitCommit
        execute printf("autocmd BufWritePost <buffer> call GitDoCommand('commit %s --cleanup=strip -F ' . shellescape(expand('%%'))) | autocmd! GitCommit * <buffer>", args)
    augroup END
endfunction
